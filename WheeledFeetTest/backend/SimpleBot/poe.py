...
def calcLegCoM(robot, motors, links):
    ...
    weightY = 0
    weightZ = 0
    Slist = [robot.motors[0].twist, robot.motors[1].twist]
    thetaList = [robot.motors[0].get_position(), robot.motors[1].get_position()]
    for i in range(len(motors)):
        motor = motors[i]
        link = links[i]
        Slist.append(motor.twist)
...
def calcLegChainIK(robot, T, rightToLeft):
    thetaList = [robot.motors[0].get_position(), 
             robot.motors[1].get_position()]
    print(thetaList)
    ...
    eomg = 0.01
    ev = 0.001
    return mr.IKinSpace(T, thetaList, eomg, ev)

...
class Robot():
    ...
    def __init__(self, is_real):
        ...
        if self.is_real:
            ...
            self.motorMovePositionScriptHandle = self.sim.getScript(self.sim.scripttype_childscript, self.sim.getObject("./base_link_respondable"))
    ...
    def sim_motors_init(self):
        ...
        for motorConfig in Config.motors:
            path = "/" + motorConfig[3]
            print(f"Beginning to stream {motorConfig[3]} (looking for object at path: '{path}')")
    ...
    def updateLeftArmCoM(self):
        motorList = [self.motors[6], self.motors[7], self.motors[8], self.motors[9], self.motors[10], self.motors[11]]
        linkList = [self.links[6], self.links[7], self.links[8], self.links[9], self.links[10], self.links[11]]
        return poe.calcLimbCoM(motorList, linkList)
    
    def updateTorsoCoM(self):
        motorList = [self.motors[12], self.motors[13], self.motors[14]]
        linkList = [self.links[12], self.links[13], self.links[14], self.links[15]]
        return poe.calcLimbCoM(motorList, linkList)
    
    def updateRightLegCoM(self):
        motorList = [self.motors[15], self.motors[16], self.motors[17], self.motors[18], self.motors[19]]
        linkList = [self.links[16], self.links[17], self.links[18], self.links[19], self.links[20]]
    ...
    def updateLeftLegCoM(self):
        ...
        linkList = [self.links[21], self.links[22], self.links[23], self.links[24], self.links[25]]
        #print(poe.calcLegCoM(self, motorList))
        return poe.calcLegCoM(self, motorList, linkList)
    ...
    def updateBalancePoint(self):
        rightAnkle = self.locate(self.motors[Config.Joints.ankle_right.value])
        leftAnkle = self.locate(self.motors[Config.Joints.ankle_left.value])
    ...
    def balanceAngleOLD(self):
        ...
        targetTheta = math.atan2(staticCoM[1] - staticKickLoc[1], staticCoM[2] - staticKickLoc[2])
        kickMotorPos = self.locate(self.motors[Config.Joints.thighlift_left.value])
        currTheta = math.atan2(self.CoM[1] - kickMotorPos[1], self.CoM[2] - kickMotorPos[2])
        thetaError = targetTheta - currTheta
